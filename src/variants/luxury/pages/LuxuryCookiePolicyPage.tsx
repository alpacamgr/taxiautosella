    ...
    <div className="min-h-screen bg-tas-paper pt-24 pb-20">
      <div className="mx-auto max-w-3xl px-6 lg:px-16">
        ...
        <div className="space-y-10 text-[15px] font-light leading-relaxed text-tas-ink/85">
          ...
          <section>
            <h2 className="font-editorial text-2xl text-tas-ink">{t('cookiePolicy.sections.own.title')}</h2>
            <p className="mt-3">
              <Trans
                i18nKey="legal:cookiePolicy.sections.own.body"
                components={{
                  code: <code className="rounded bg-tas-parchment px-1.5 py-0.5 text-[13px]" />,
                }}
              />
            </p>
          ...
          <section>
            <h2 className="font-editorial text-2xl text-tas-ink">{t('cookiePolicy.sections.thirdParty.title')}</h2>
          ...
          <section>
            <h2 className="font-editorial text-2xl text-tas-ink">{t('cookiePolicy.sections.hosting.title')}</h2>
            <p className="mt-3">{t('cookiePolicy.sections.hosting.body')}</p>
          </section>

          <section>
            <h2 className="font-editorial text-2xl text-tas-ink">{t('cookiePolicy.sections.manage.title')}</h2>
            <p className="mt-3">{t('cookiePolicy.sections.manage.body')}</p>
            <button
              type="button"
              onClick={openCookieSettings}
              className="mt-4 inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-tas-ink/20 bg-tas-surface px-6 text-xs font-semibold uppercase tracking-[0.08em] text-tas-ink transition-colors hover:border-tas-accent-strong hover:text-tas-accent-strong focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tas-focus"
            >
