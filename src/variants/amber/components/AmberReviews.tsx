...
export const AmberReviews: React.FC = () => {
          ...
          <header className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div className="max-w-2xl">
              <h2 className="text-3xl font-bold tracking-tight text-tas-ink sm:text-4xl">
                <Trans i18nKey="reviews:title" components={{ em: <span className="text-tas-accent-on-light" /> }} />
              </h2>
            </div>
            ...
            <Reveal key={r.id} delay={i * 80}>
              <figure className="flex h-full flex-col justify-between rounded-xl border border-tas-line bg-tas-surface p-6 shadow-card">
                ...
                <figcaption className="mt-6 flex items-center gap-3 border-t border-tas-line pt-4">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-tas-primary-soft text-sm font-bold text-tas-accent-on-light">
                    {r.name.charAt(0)}
                  </span>
                  <span>
