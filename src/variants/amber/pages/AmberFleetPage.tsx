...
export const AmberFleetPage: React.FC = () => {
                  ...
                  <div className="flex h-full flex-col justify-between rounded-xl border border-tas-line bg-tas-surface p-6 shadow-card transition-shadow hover:shadow-card-hover">
                    <div>
                      <div className="relative mb-5 h-56 overflow-hidden rounded-lg bg-tas-parchment">
                        ...
                        />
                        {badge && (
                          <span className="absolute right-3 top-3 rounded-full bg-tas-primary px-3 py-1 text-xs font-bold text-tas-on-accent">
                            {badge}
                          </span>
                        )}
                      </div>
                      <h3 className="text-xl font-bold text-tas-ink">{name}</h3>
                      <p className="mt-1 text-sm font-semibold text-tas-accent-on-light">{subtitle}</p>
                        ...
                        {Array.isArray(features) &&
                          features.map((feat, idx) => (
                            <li key={idx} className="flex items-start gap-2">
                              <Check className="mt-0.5 h-4 w-4 flex-none text-tas-accent-on-light" />
                              <span>{feat}</span>
                            </li>
                          ))}
                      </ul>
                    ...
                    <div className="mt-6 flex flex-col gap-4 border-t border-tas-line pt-5">
                      ...
                      <button
                        ...
                        className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-tas-primary px-5 text-sm font-bold text-tas-on-accent transition-colors hover:bg-tas-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tas-focus focus-visible:ring-offset-2"
                      >
