...
import { AmberReviews } from '../components/AmberReviews';

const em = { em: <span className="text-tas-accent-on-light" /> };
...
const secondaryButton =
  'inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-tas-line bg-tas-surface px-5 text-sm font-semibold text-tas-ink transition-colors hover:border-tas-ink/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tas-focus focus-visible:ring-offset-2';
const textLink =
  'inline-flex items-center gap-1.5 text-sm font-semibold text-tas-accent-on-light transition-colors hover:text-tas-accent-strong';
const inputClass =
  'min-h-12 w-full rounded-lg border border-tas-line bg-tas-surface px-3.5 text-sm text-tas-ink placeholder:text-tas-muted-text focus:border-tas-accent-on-light focus:outline-none focus:ring-2 focus:ring-tas-focus/25';

export const AmberHome: React.FC = () => {
  const { openInquiryModal, setHeroPrefill } = useAppStore();
  const { t } = useTranslation(['homeAmber', 'home', 'fleet', 'members']);
  ...
  const handleRouteClick = (id: (typeof ROUTE_IDS)[number]) => {
    setHeroPrefill({
      pickup: t(`home:routes.items.${id}.from`),
      destination: t(`home:routes.items.${id}.to`),
      date: '',
      passengers: '2',
    });
    navigate('/booking');
  ...
  return (
    <div>
      ...
      <section className="relative bg-tas-ink text-white">
        <div className="absolute inset-0">
          <ResponsiveImage
            ...
            alt={t('homeAmber:hero.backgroundAlt')}
            priority
            sizes="100vw"
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-6 pb-0 pt-12 sm:pt-16 lg:min-h-[820px] lg:grid-cols-12 lg:items-start lg:gap-12 lg:px-16 lg:pb-24 lg:pt-16">
          <div className="lg:col-span-7 [text-shadow:0_2px_14px_rgba(12,24,36,0.55)]">
            <h1 className="max-w-2xl text-4xl font-bold leading-[1.08] tracking-tight sm:text-5xl lg:text-6xl">
              <Trans i18nKey="homeAmber:hero.title" components={{ em: <span className="text-tas-accent-on-dark" /> }} />
            </h1>
            <p className="mt-5 max-w-xl text-base leading-relaxed text-white/85 sm:text-lg">
              <span className="sm:hidden">{t('homeAmber:hero.subtitleShort')}</span>
              <span className="hidden sm:inline">{t('homeAmber:hero.subtitle')}</span>
            </p>
          ...
          <div className="-mb-60 lg:col-span-5 lg:mb-0">
            ...
            >
              <h2 className="text-xl font-bold tracking-tight">{t('homeAmber:quote.title')}</h2>

              <div className="mt-5 space-y-4">
                <label className="block">
                  <span className="mb-1.5 block text-sm font-semibold">{t('homeAmber:quote.pickupLabel')}</span>
                  <span className="relative block">
                    <MapPin className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-tas-accent-on-light" />
                    <input
                      type="text"
                      className={`${inputClass} pl-10`}
                      placeholder={t('homeAmber:quote.pickupPlaceholder')}
                ...
                <label className="block">
                  <span className="mb-1.5 block text-sm font-semibold">{t('homeAmber:quote.destinationLabel')}</span>
                  <span className="relative block">
                    <MapPin className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-tas-accent-on-light" />
                    <input
                      type="text"
                      className={`${inputClass} pl-10`}
                      placeholder={t('homeAmber:quote.destinationPlaceholder')}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <label className="block">
                    <span className="mb-1.5 block text-sm font-semibold">{t('homeAmber:quote.dateLabel')}</span>
                    <span className="relative block">
                      <CalendarDays className="pointer-events-none absolute left-3.5 top-1/2 hidden h-4 w-4 -translate-y-1/2 text-tas-accent-on-light sm:block" />
                      <input
                        type="date"
                  ...
                  <div>
                    <span className="mb-1.5 block text-sm font-semibold">{t('homeAmber:quote.passengersLabel')}</span>
                    <div className="flex min-h-12 items-center justify-between rounded-lg border border-tas-line bg-tas-surface px-1.5">
                      <button
                        type="button"
                        aria-label={t('homeAmber:quote.decrease')}
                        onClick={() => setBooking({ ...booking, passengers: Math.max(1, booking.passengers - 1) })}
                        className="flex h-9 w-9 items-center justify-center rounded-md text-tas-ink transition-colors hover:bg-tas-parchment focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tas-focus"
                      >
                        <Minus className="h-4 w-4" />
                      </button>
                      <span className="inline-flex items-center gap-1.5 text-sm font-bold" aria-live="polite">
                        <Users className="h-4 w-4 text-tas-accent-on-light" />
                        {booking.passengers}
                      </span>
                      <button
                        type="button"
                        aria-label={t('homeAmber:quote.increase')}
                ...
                <button
                  ...
                  className="w-full text-center text-sm font-semibold text-tas-accent-on-light underline-offset-4 hover:underline"
                >
                  {t('homeAmber:quote.whatsappFallback')}
                </button>
              </div>

              <p className="mt-4 border-t border-tas-line pt-4 text-center text-xs text-tas-muted-text">
                {t('homeAmber:quote.trustLine')}
              </p>
            </form>
          </div>
        </div>
      ...
      {/* GOOGLE RATING BAND — absorbs the booking card overhang on phones */}
      <section className="bg-tas-ink text-white">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-1 px-6 pb-7 pt-72 text-center lg:flex-row lg:justify-center lg:gap-4 lg:px-16 lg:py-5 lg:text-left">
          <span className="flex items-center gap-2">
            <span className="flex" aria-hidden="true">
              {[0, 1, 2, 3, 4].map((n) => (
                <Star key={n} className="h-5 w-5 fill-tas-star text-tas-star" />
              ))}
            </span>
            <span className="text-lg font-bold">{t('homeAmber:ratingBand.score')}</span>
          </span>
          <span className="text-sm text-white/80">{t('homeAmber:ratingBand.body')}</span>
        </div>
      </section>

      {/* TRUST */}
      <section className="border-b border-tas-line bg-tas-surface">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-x-4 gap-y-8 px-6 py-12 lg:grid-cols-4 lg:px-16 lg:py-12">
          {trust.map(({ id, Icon }, i) => (
            <Reveal key={id} delay={i * 60} className="flex flex-col gap-3 sm:flex-row sm:gap-4">
              ...
              <span>
                <span className="block text-base font-bold text-tas-ink">{t(`homeAmber:trust.${id}.title`)}</span>
                <span className="mt-1 block text-sm leading-relaxed text-tas-muted-text">{t(`homeAmber:trust.${id}.body`)}</span>
              </span>
            </Reveal>
          ))}
        </div>
      ...
      <section className="bg-tas-surface py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-16">
          <Reveal className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-3xl font-bold tracking-tight text-tas-ink sm:text-4xl">{t('homeAmber:routes.title')}</h2>
              <p className="mt-3 max-w-2xl text-base text-tas-muted-text">{t('homeAmber:routes.subtitle')}</p>
            </div>
            <Link to="/booking" className={textLink}>
              {t('homeAmber:routes.viewAll')}
              <ArrowRight className="h-4 w-4" />
            </Link>
          </Reveal>
      ...
      <section className="border-b border-tas-line bg-tas-parchment py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-16">
          <Reveal>
            <h2 className="text-3xl font-bold tracking-tight text-tas-ink sm:text-4xl">{t('homeAmber:services.title')}</h2>
            <p className="mt-3 max-w-2xl text-base text-tas-muted-text">{t('homeAmber:services.subtitle')}</p>
          </Reveal>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            <Reveal className="flex flex-col overflow-hidden rounded-xl border border-tas-line bg-tas-surface shadow-card transition-shadow hover:shadow-card-hover">
              <ResponsiveImage
                ...
                alt={t('homeAmber:services.transfers.title')}
                sizes="(min-width: 768px) 33vw, 100vw"
                className="h-52 w-full object-cover"
              />
              <div className="flex flex-1 flex-col p-6">
                <h3 className="text-xl font-bold text-tas-ink">{t('homeAmber:services.transfers.title')}</h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-tas-muted-text">{t('homeAmber:services.transfers.body')}</p>
                <Link to="/booking" className={`${primaryButton} mt-6 self-start`}>
                  {t('homeAmber:services.transfers.cta')}
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </div>
            </Reveal>
            ...
            <Reveal delay={80} className="flex flex-col overflow-hidden rounded-xl border border-tas-line bg-tas-surface shadow-card transition-shadow hover:shadow-card-hover">
              <ResponsiveImage
                ...
                alt={t('homeAmber:services.excursions.title')}
                sizes="(min-width: 768px) 33vw, 100vw"
                className="h-52 w-full object-cover"
              />
              <div className="flex flex-1 flex-col p-6">
                <h3 className="text-xl font-bold text-tas-ink">{t('homeAmber:services.excursions.title')}</h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-tas-muted-text">{t('homeAmber:services.excursions.body')}</p>
                <Link to="/excursions" className={`${secondaryButton} mt-6 self-start`}>
                  {t('homeAmber:services.excursions.cta')}
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </div>
            </Reveal>
            ...
            <Reveal delay={160} className="flex flex-col overflow-hidden rounded-xl border border-tas-line bg-tas-surface shadow-card transition-shadow hover:shadow-card-hover">
              <ResponsiveImage
                ...
                alt={t('homeAmber:services.local.title')}
                sizes="(min-width: 768px) 33vw, 100vw"
                className="h-52 w-full object-cover"
              />
              <div className="flex flex-1 flex-col p-6">
                <h3 className="text-xl font-bold text-tas-ink">{t('homeAmber:services.local.title')}</h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-tas-muted-text">{t('homeAmber:services.local.body')}</p>
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <a
                    href={whatsappLink(t('homeAmber:urgent.whatsappMessage'))}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`${primaryButton} w-full sm:w-auto`}
                  >
                    <MessageSquare className="h-4 w-4" />
                    {t('homeAmber:services.local.cta')}
                  </a>
                  <a href={`tel:${PHONE_TEL}`} className={`${secondaryButton} w-full sm:w-auto`}>
                    <Phone className="h-4 w-4 text-tas-accent-on-light" />
                    {t('homeAmber:services.local.call', { phone: PHONE_DISPLAY })}
                  </a>
                </div>
              </div>
            </Reveal>
      ...
      <section className="border-t border-tas-line bg-tas-surface py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-16">
          <Reveal>
            <h2 className="text-3xl font-bold tracking-tight text-tas-ink sm:text-4xl">{t('homeAmber:steps.title')}</h2>
            <p className="mt-3 text-base text-tas-muted-text">{t('homeAmber:steps.subtitle')}</p>
          </Reveal>
          <ol className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {steps.map(({ id, Icon }, i) => (
              <Reveal key={id} as="li" delay={i * 80} className="relative">
                <div className="flex items-center gap-3">
                  <span className="flex h-11 w-11 items-center justify-center rounded-full bg-tas-primary text-tas-on-accent">
                    <Icon className="h-5 w-5" />
                  </span>
                  <span className="text-sm font-bold text-tas-accent-on-light">0{i + 1}</span>
                </div>
                <h3 className="mt-4 text-lg font-bold text-tas-ink">{t(`homeAmber:steps.items.${id}.title`)}</h3>
                <p className="mt-2 text-sm leading-relaxed text-tas-muted-text">{t(`homeAmber:steps.items.${id}.body`)}</p>
              </Reveal>
            ))}
          </ol>
        </div>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-8 px-6 py-12 lg:grid-cols-4 lg:px-16">
          {[
            ...
            <div key={s.id}>
              ...
              <div className="mt-1 text-sm font-semibold text-tas-muted-text">{t(`homeAmber:stats.${s.id}`)}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-tas-surface py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-16">
          <Reveal className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-3xl font-bold tracking-tight text-tas-ink sm:text-4xl">{t('homeAmber:drivers.title')}</h2>
              <p className="mt-3 max-w-2xl text-base text-tas-muted-text">{t('homeAmber:drivers.subtitle')}</p>
            </div>
            <Link to="/members" className={textLink}>
              {t('homeAmber:drivers.viewAll')}
              <ArrowRight className="h-4 w-4" />
            </Link>
          </Reveal>
            ...
            {FEATURED_DRIVERS.map((driver, i) => (
              <Reveal key={driver} delay={i * 60}>
                <button
                  ...
                  onClick={() =>
                    openInquiryModal(
                      t('homeAmber:drivers.requestContext'),
                      t('homeAmber:drivers.requestPrefill', { driver }),
                    )
                  }
                  className="group flex w-full items-center gap-4 rounded-xl border border-tas-line bg-tas-surface p-5 text-left shadow-card transition-shadow hover:shadow-card-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tas-focus focus-visible:ring-offset-2"
                >
                  <span className="relative flex h-14 w-14 flex-none items-center justify-center overflow-hidden rounded-full bg-tas-primary-soft text-base font-bold text-tas-accent-on-light">
                    {initials(driver)}
                    <ResponsiveImage
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-base font-bold text-tas-ink group-hover:text-tas-accent-on-light">{driver}</span>
                    <span className="mt-1 block text-sm text-tas-muted-text">
                      {t('homeAmber:drivers.based')} {t('homeAmber:drivers.basedValue')}
                    </span>
                    <span className="block text-sm text-tas-muted-text">
                      {t('homeAmber:drivers.languages')}: {t('homeAmber:drivers.languagesValue')}
                    </span>
                  </span>
                  <ArrowRight className="h-4 w-4 flex-none text-tas-muted-text group-hover:text-tas-accent-on-light" />
                </button>
              </Reveal>
            ))}
          </div>
      ...
      <section className="border-t border-tas-line bg-tas-surface py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-16">
          <Reveal className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-3xl font-bold tracking-tight text-tas-ink sm:text-4xl">{t('homeAmber:fleet.title')}</h2>
              <p className="mt-3 max-w-2xl text-base text-tas-muted-text">{t('homeAmber:fleet.subtitle')}</p>
            </div>
            <Link to="/fleet" className={textLink}>
              {t('homeAmber:fleet.viewAll')}
              <ArrowRight className="h-4 w-4" />
            </Link>
          </Reveal>
            ...
            {FLEET.slice(0, 3).map((vehicle, i) => {
              ...
              return (
                <Reveal key={vehicle.id} delay={i * 80} className="flex flex-col rounded-xl border border-tas-line bg-tas-surface p-5 shadow-card transition-shadow hover:shadow-card-hover">
                  ...
                  <p className="text-sm font-semibold text-tas-accent-on-light">{subtitle}</p>
                  ...
                  <div className="mt-5 flex items-center justify-between border-t border-tas-line pt-4">
                    <div className="flex items-center gap-4 text-sm font-medium text-tas-ink">
                      ...
                      <span className="inline-flex items-center gap-1.5">
                        <Briefcase className="h-4 w-4 text-tas-accent-on-light" />
                    ...
                    <button
                      ...
                      className="flex h-10 w-10 items-center justify-center rounded-full bg-tas-primary text-tas-on-accent transition-colors hover:bg-tas-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tas-focus focus-visible:ring-offset-2"
                    >
                      <ArrowRight className="h-4 w-4" />
                    </button>
                  </div>
      ...
      <section className="border-t border-tas-line bg-tas-surface py-16 sm:py-20">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-12 lg:px-16">
          <Reveal className="lg:col-span-6">
            <h2 className="text-3xl font-bold tracking-tight text-tas-ink sm:text-4xl">{t('homeAmber:partners.title')}</h2>
            <p className="mt-3 max-w-xl text-base leading-relaxed text-tas-muted-text">{t('homeAmber:partners.body')}</p>
            <button
              type="button"
              onClick={() => openInquiryModal(t('homeAmber:partners.context'), t('homeAmber:partners.prefill'))}
              className={`${primaryButton} mt-6`}
            >
              {t('homeAmber:partners.cta')}
              <ArrowRight className="h-4 w-4" />
            </button>
          </Reveal>
          <Reveal delay={80} className="lg:col-span-6">
            <ResponsiveImage
              ...
              alt={t('homeAmber:partners.title')}
              sizes="(min-width: 1024px) 50vw, 100vw"
              className="mb-6 h-56 w-full rounded-xl object-cover"
            />
            <h3 className="text-sm font-bold text-tas-muted-text">{t('homeAmber:partners.listTitle')}</h3>
              ...
              {PARTNER_LINKS.map((p) => (
                <li key={p.key}>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex min-h-12 items-center rounded-lg border border-tas-line bg-tas-surface px-4 text-sm font-semibold text-tas-ink transition-colors hover:border-tas-accent-on-light hover:text-tas-accent-on-light"
                  >
      ...
      <section className="bg-tas-surface px-6 pb-16 sm:pb-20 lg:px-16">
        <Reveal className="mx-auto flex max-w-7xl flex-col gap-6 rounded-2xl bg-tas-ink p-8 text-white sm:p-10 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">{t('homeAmber:urgent.title')}</h2>
            <p className="mt-2 max-w-xl text-base text-white/80">{t('homeAmber:urgent.body')}</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <a href={`tel:${PHONE_TEL}`} className={`${primaryButton} min-h-14 w-full px-6 text-base sm:w-auto`}>
              <Phone className="h-5 w-5" />
              {t('homeAmber:urgent.call', { phone: PHONE_DISPLAY })}
            </a>
            <a
              href={whatsappLink(t('homeAmber:urgent.whatsappMessage'))}
              target="_blank"
