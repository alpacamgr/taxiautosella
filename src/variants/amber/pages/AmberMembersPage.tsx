...
export const AmberMembersPage: React.FC = () => {
        ...
        <Reveal>
          <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex items-center gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-full bg-tas-primary-soft text-tas-accent-on-light">
            ...
            <Reveal key={driver} delay={(i % 3) * 50}>
              <article className="flex h-full flex-col justify-between rounded-xl border border-tas-line bg-tas-surface p-5 shadow-card transition-shadow hover:shadow-card-hover">
                <div className="flex items-center gap-4">
                  <div
                    className="relative flex h-14 w-14 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-tas-primary-soft text-base font-bold text-tas-accent-on-light"
                    aria-hidden="true"
                  >
                    {initials(driver)}
                    {i < 6 && (
                ...
                <button
                  ...
                  className="mt-5 inline-flex items-center gap-1.5 self-start text-sm font-semibold text-tas-accent-on-light hover:text-tas-accent-strong"
                >
              ...
              {PARTNERS.map((partner, i) => {
                ...
                return (
                  <Reveal key={partner.id} delay={(i % 3) * 40}>
                    ...
                    >
                      <div className="min-w-0">
                        <h4 className="text-sm font-bold text-tas-ink group-hover:text-tas-accent-on-light">
                          {name}
                        </h4>
                        <p className="mt-0.5 text-sm text-tas-muted-text">{desc}</p>
                      </div>
                      <ArrowRight className="h-4 w-4 flex-shrink-0 text-tas-muted-text group-hover:text-tas-accent-on-light" />
                    </a>
                  </Reveal>
                );
              })}
          ...
          <div>
            ...
            <div className="mb-8 flex flex-wrap gap-2.5">
              {VENUES.map((venue) => (
                <button
                  ...
                  className="rounded-full border border-tas-line bg-tas-surface px-3.5 py-1.5 text-sm font-medium text-tas-ink transition-colors hover:border-tas-accent-on-light hover:text-tas-accent-on-light"
                >
                  {venue}
                </button>
              ))}
            ...
            <Reveal>
              <div className="rounded-2xl bg-tas-ink p-6 text-white sm:p-8">
                ...
                <a
                  href={`tel:${PHONE_TEL}`}
                  className="mt-6 inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-tas-primary px-5 text-sm font-bold text-tas-on-accent transition-colors hover:bg-tas-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-tas-ink"
                >
